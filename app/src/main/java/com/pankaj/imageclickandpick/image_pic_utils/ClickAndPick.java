...
import static com.pankaj.imageclickandpick.image_pic_utils.ClickAndPickFiles.singleFileList;


@SuppressWarnings({"unused", "FieldCanBeLocal", "ResultOfMethodCallIgnored"})
public class ClickAndPick implements Constants {
    ...
    private static Uri createCameraPictureFile(@NonNull Context context) throws IOException {
        File imagePath = ClickAndPickFiles.getCameraPicturesLocation(context);
        Uri uri = ClickAndPickFiles.getUriToFile(context, imagePath);
    ...
    public static void openChooserWithDocuments(Fragment fragment, @Nullable String chooserTitle, int type) {
        try {
            Intent intent = createChooserIntent(Objects.requireNonNull(fragment.getActivity()), chooserTitle, type);
    ...
    public static void handleActivityResult(int requestCode, int resultCode, Intent data, Activity activity, @NonNull Callbacks callbacks) {
        boolean isClickAndPick = (requestCode & RequestCodes.ClickAndPick_IDENTIFICATOR) > 0;
        if (isClickAndPick) {
    ...
    private static void onPictureReturnedFromDocuments(Intent data, Activity activity, @NonNull Callbacks callbacks) {
        try {
            ...
            File photoFile = ClickAndPickFiles.pickedExistingPicture(activity, photoPath);
            callbacks.onImagesPicked(singleFileList(photoFile), ImageSource.DOCUMENTS, restoreType(activity));

            if (configuration(activity).shouldCopyPickedImagesToPublicGalleryAppFolder()) {
                ClickAndPickFiles.copyFilesInSeparateThread(activity, singleFileList(photoFile));
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static void onPictureReturnedFromGallery(Intent data, Activity activity, @NonNull Callbacks callbacks) {
        try {
            ...
            if (clipData == null) {
                ...
                File file = ClickAndPickFiles.pickedExistingPicture(activity, uri);
                files.add(file);
            } else {
            ...
            if (configuration(activity).shouldCopyPickedImagesToPublicGalleryAppFolder()) {
                ClickAndPickFiles.copyFilesInSeparateThread(activity, files);
            }
    ...
    private static void onPictureReturnedFromCamera(Activity activity, @NonNull Callbacks callbacks) {
        try {
            ...
            File photoFile = ClickAndPick.takenCameraPicture(activity);
            List<File> files = new ArrayList<>();
            files.add(photoFile);
    ...
    public static ClickAndPickConfiguration configuration(@NonNull Context context) {
        return new ClickAndPickConfiguration(context);
    }
}